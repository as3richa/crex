...
run(void *contexts, void *matches, void *regex, const char *str, size_t size, void *allocator) {
  ...
  const int status = pcre2_match(regex, (const unsigned char *)str, size, 0, 0, matches, context);
  assert(status >= 0 || status == PCRE2_ERROR_NOMATCH);

  return 1;
}

const execution_engine_t ex_pcre_default = {
    "pcre-default", 1, 1, create, destroy, compile_regex, destroy_regex, run};
